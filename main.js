function canvas() {
        ...
        if (countdown < 0) {
            ...
            timerInterval = setInterval(() => {
                ...
                if (gameTime <= 0) {
                    ...
                    showGameOverPopup(username,benderamusuh,benderasendiri,player1Score,player2Score);
                }
            }, 1000);
        }
    }, 1000);
    ...
    var player1FallingDownImages = [];
    for(var i = 0; i <= 4; i++ ){
        var img = new Image();
        img.src = `Characters/Character 0${player1Number} - ${benderasendiri}/Falling Down/Falling Down_${i.toString().padStart(3, '0')}.png`;
        player1FallingDownImages.push(img);
    }

    var player1JumpImages = [];
    for(var i = 0; i <= 4; i++){
        var img = new Image();
        img.src = `Characters/Character 0${player1Number} - ${benderasendiri}/Jump/Jump_${i.toString().padStart(3, '0')}.png`;
        player1JumpImages.push(img);
    }
    ...
    var isJump2 = false;
    var player2JumpImages = [];
    for (var i = 0; i <= 4; i++) {
        var img = new Image();
        img.src = `Characters/Character 0${player2Number} - ${benderamusuh}/Jump/Jump_${i.toString().padStart(3, '0')}.png`;
        player2JumpImages.push(img);
    }

    var player2FallingDownImages2 = [];
for (var i = 0; i <= 4; i++) {
    var img = new Image();
    img.src = `Characters/Character 0${player2Number} - ${benderamusuh}/Falling Down/Falling Down_${i.toString().padStart(3, '0')}.png`;
    player2FallingDownImages2.push(img);
}
    ...
    var player2JumpImages = 0;
    var player2FallingDownIndex = 0;
    var player2FallingDownImages = 0;
    var isFallingDown2 = false;
    var player2JumpIndex = 0;
    ...
    var player1FallingDownIndex = 0;
    var player1MoveBackwardIndex = 0;
    var player1JumpIndex = 0;
    var isKicking = false;
...
for (var x = sectionStartX; x < sectionEndX; x += flagWidth) {
    ...
    if (isKicking) {
        ...
        ctx.drawImage(player1FallingDownImages[player1FallingDownIndex], player1x, player1y, 155, 155); 
    }else if(isJump){
        ctx.drawImage(player1JumpImages[player1JumpIndex], player1x, player1y, 155, 155); 
    } else {
    ...
    }else if(isFallingDown2){
        ctx.drawImage(player2FallingDownImages[player2FallingDownIndex], -77.5, -77.5, 155, 155); 
    }else {
    ...
    function animatePlayers() {
        if (isKicking) {
            ...
            player1FallingDownIndex++;
        if (player1FallingDownIndex >= player1FallingDownImages.length) {
            player1FallingDownIndex = 0;
            isFallingDown = false;
        }
        } else if(isJump){
            player1JumpIndex++;
            if (player1JumpIndex >= player1JumpImages.length) {
                player1JumpIndex = 0;
                isJump = false;
                isFallingDown = true;
                player1y += 40; // Reset player1's y position
            }
        }else {
            player1IdleIndex++;
        ...
        if (isKickingPlayer2) {
            ...
            player2JumpIndex++;
            if (player2JumpIndex >= player2JumpImages.length) {
                player2JumpIndex = 0;
                isJump2 = false;
                isFallingDown2 = true;
            }
        } else if (isFallingDown2) {
            player2FallingDownIndex++;
            if (player2FallingDownIndex >= player2FallingDownImages2.length) {
                player2FallingDownIndex = 0;
    ...
    document.addEventListener('keydown', function(event) {
        switch (event.key) {
            ...
            if (!isJump && !isFallingDown) { // Start jump only if not already jumping or falling
                isJump = true;
                player1JumpIndex = 0;
                player1y -= 40;
            }
            ...
            if (!isJump2 && !isFallingDown2) { // Start jump only if not already jumping or falling
                isJump2 = true;
                player2JumpIndex = 0;
                player2y -= 40; // Move player up
            }
            break;
            case 'ArrowDown': // Move player2 down
                player2y += 10;
                break;
            case 'Enter': // Player2 kick
    ...
    function showGameOverPopup(username, benderasendiri,benderamusuh,player1Score,player2Score) {
        ...
        gameOverPopup.innerHTML = `
            <div class="popup-inner">
                ...
                <p class="popup-text">${username}</p>
                 <div class="bendera-all">
                  
                  <p class="score1">${player1Score} :  ${player2Score}</p> 
            
                <img class="bendera-sendiri" src="./Flag/${benderasendiri}.png">      
                <img class="bendera-musuh" src="./Flag/${benderamusuh}.png">
                </div>

                <div class="all-btn">
                    <button type="submit" class="restart-button" id="restartButton">Restart Game</button>
                <button class="save-button">Save Match</button>
                 </div>
            </div>
        `;
